...
public class ClientComms { 
	...
	final static String className = "ClientComms";
	...
	// force to close all the Thread in Mqtt.
	...
	public void shutdownConnection(MqttToken token, MqttException reason) {
		try {
			...
			//YLogger.e("ClientComms", "shutdownConnection", e);
		}
	}
	...
	//	log.fine(className,methodName,"222");
		//YLogger.d(className, "Action handleOldTokens");
	...
	private class DisconnectBG implements Runnable {
		...
		public void run() {
			...
			try {
				...
				token.internalTok.waitUntilSent(3000);
			}
			catch (Exception ex) {
			  // YLogger.e(className, "run", ex);
			}
			finally {
				try {
