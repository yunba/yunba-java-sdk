...
public class MqttAsyncClient implements IMqttAsyncClient { // DestinationProvider {
	...
	public static ConcurrentHashMap<Integer, IMqttActionListener> callbacks
	= new ConcurrentHashMap<Integer, IMqttActionListener>();
	...
	public static MqttAsyncClient createMqttClient(String appkey) throws MqttException, JSONException {
				...
				if(MqttUtil.isEmpty(clientId)) throw ExceptionHelper.createMqttException(MqttException.REASON_CODE_INVALID_CLIENT_ID);
				if(MqttUtil.isEmpty(serverURI)) throw ExceptionHelper.createMqttException(MqttException.REASON_CODE_BROKER_UNAVAILABLE);
...
//				}
	...
	public IMqttToken connect(IMqttActionListener callback) throws MqttException, MqttSecurityException {
...
//	}
	...
	public void publishToAlias(String topic, String payload, IMqttActionListener callback) throws MqttPersistenceException, UnsupportedEncodingException, Exception {
		publish(",yta/" + topic, payload.getBytes("utf-8"), 1, false, null, callback);
	}
	
	public void setAlias(String alias, IMqttActionListener callback) throws Exception {
		publish(TOPIC_SET_ALIAS, alias.getBytes("utf-8"),  1, false, null, callback);
	}

//	public void getAlias(IMqttActionListener callback) throws Exception {
//		publish(TOPIC_GET_ALIAS, "".getBytes("utf-8"), 1 , false, null, callback);		
//	}

	
	public void getAliassByTopic(String topic, IMqttActionListener callback) throws MqttException {
	...
	public void publish2(String topic, String msg, JSONObject opts, final IMqttActionListener callback) throws Exception {

		MqttToken token = new MqttToken(getClientId());
		MqttExpand expand = new MqttExpandPublish(topic, msg, MqttExpand.CMD_PUBLISH, opts);
		if (null != callback) {
			int id = getCallbackID();
			callbacks.put(id, callback);
			token.setUserContext(id);
		...
		public void getStatusByAlias(String topic, IMqttActionListener callback) throws MqttException {
			MqttToken token = new MqttToken(getClientId());
		...
		public void getTopics(String alias, IMqttActionListener callback) throws MqttException {
			MqttToken token = new MqttToken(getClientId());
