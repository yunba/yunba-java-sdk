 ...
 * interface allowing applications to initiate MQTT actions and then carry
...
public class MqttAsyncClient implements IMqttAsyncClient { // DestinationProvider
	...
	public static final String TOPIC_SET_ALIAS = ",yali";
	public static final String TOPIC_GET_ALIAS = ",yaliget";
	...
	/**
	 * Create an MqttAsyncClient that can be used to communicate with an MQTT
	 * server.
	 * <p>
	 * The address of the server should be a URI, using a scheme of either
	 * "tcp://" for a TCP connection or "ssl://" for a TCP connection secured by
	 * SSL/TLS. For example:
	 * <ul>
	 ...
	 * </p>
	 * <p>
	 * A client identified to connect to an MQTT server, it must be unique
	 * across all clients connecting to the same server. A convenience method is
	 * provided to generate a random client id that should satisfy this criteria
	 * - {@link #generateClientId()}. As the client identifier is used by the
	 * server to identify a client when it reconnects, the client must use the
	 * same identifier between connections if durable subscriptions are used and
	 ...
	 * failure in the client or server can result in lost messages. A pluggable
	...
	public MqttAsyncClient(String serverURI, String clientId,
		...
		log.fine(className, methodName, "101", new Object[] { clientId,
	...
	public static MqttAsyncClient createMqttClient(String appkey)
		...
		return new MqttAsyncClient(serverURI, clientId, null);
	}
	...
	public IMqttToken disconnect(long quiesceTimeout, Object userContext,
		...
		log.fine(className, methodName, "104", new Object[] {
	...
	public IMqttToken subscribe(String[] topicFilters, int[] qos,
		...
		log.fine(className, methodName, "106", new Object[] { subs,
				userContext, callback });
	...
	public IMqttToken unsubscribe(String[] topicFilters, Object userContext,
		...
		log.fine(className, methodName, "107", new Object[] { subs,
				userContext, callback });
	...
	public IMqttDeliveryToken publish(String topic, MqttMessage message,
		...
		log.fine(className, methodName, "111", new Object[] { topic,
				userContext, callback });
	...
	public static boolean isValidTopic(String s) {
		if (null == s)
			return false;
		Pattern p = Pattern.compile("^[0-9a-zA-Z_]{0,50}$");
	...
	public MqttToken ping(IMqttActionListener callback) throws MqttException {
		MqttToken token = new MqttToken(getClientId());
		token.setActionCallback(callback);
		MqttPingReq pingCommand = new MqttPingReq();
		comms.sendNoWait(pingCommand, token);
		return token;
	...
	// public MqttToken expand(MQTTMessage cache) throws MqttException {
	// MqttToken token = new MqttToken(getClientId());
	// MqttExpand expand = new MqttExpand(cache.topic, cache.EXPAND_COMMNAD);
	// token.setUserContext(cache.callbackId);
	// comms.sendNoWait(expand, token);
	// return token;
	//
	...
	public void publishToAlias(String topic, String payload,
			IMqttActionListener callback) throws MqttPersistenceException,
			UnsupportedEncodingException, Exception {
		publish(",yta/" + topic, payload.getBytes("utf-8"), 1, false, null,
				callback);
	}

	public void setAlias(String alias, IMqttActionListener callback)
			throws Exception {
		publish(TOPIC_SET_ALIAS, alias.getBytes("utf-8"), 1, false, null,
				callback);
	}

	// public void getAlias(IMqttActionListener callback) throws Exception {
	// publish(TOPIC_GET_ALIAS, "".getBytes("utf-8"), 1 , false, null,
	// callback);
	// }

	public void getAliassByTopic(String topic, IMqttActionListener callback)
			throws MqttException {
		...
		if (null != callback) {
			...
			token.setUserContext(id);
		}

		comms.sendNoWait(expand, token);
	}

	public void publish2(String topic, String msg, JSONObject opts,
	...
	public void subscribePresence(String topic,
		...
		topic += "/p";
		try {
			subscribe(topic, 1, null, mqttAction);
	...
	public void unsubscribePresence(String topic,
			final IMqttActionListener mqttAction) {
		if (!isValidTopic(topic)) {
		...
		topic += "/p";
		try {
			unsubscribe(topic, null, mqttAction);
		} catch (MqttException e) {
			executor.execute(new Runnable() {
				public void run() {
					mqttAction.onFailure(null, new MqttException(
